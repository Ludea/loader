...
export default class BLPImage {
	...
	constructor(data: BufferWrapper) {
		...
		if (this.encoding === 1)
			...
			this.palette = new ArrayBuffer(0);
	}
	 ...
	 * @private
	 */
	_getCompressed(canvasData: ImageData | ArrayBuffer | Uint8Array | Uint8ClampedArray | null | undefined, mask = 0b1111): BufferWrapper {
		const flags = this.alphaDepth > 1 ? (this.alphaEncoding === 7 ? DXT5 : DXT3) : DXT1;
		const data = canvasData ? canvasData : new ArrayBuffer(this.scaledWidth * this.scaledHeight * 4);
	 ...
	 * @private
	 */
	_marshalBGRA(canvasData: ImageData | ArrayBuffer | Uint8ClampedArray | Uint8Array | null, mask: number): BufferWrapper | undefined {
